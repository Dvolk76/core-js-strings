...
function repeatString(str, times) {
  ...
  return str.repeat(Math.abs(times));
}
...
function removeFirstOccurrences(str, value) {
  const indexToRemove = str.indexOf(value);

  if (indexToRemove !== -1) {
    const newStr =
      str.slice(0, indexToRemove) + str.slice(indexToRemove + value.length);
    return newStr;
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  const indexToRemove = str.lastIndexOf(value);
...
function countVowels(str) {
  ...
  const counter = str.match(vowels);
  if (counter === null) {
    return 0;
  }
